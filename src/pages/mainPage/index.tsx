...
const MainPage: FC<any> = (props) => {
  ...
  const onDrop = (e) => {
    ...
    if (isCreate) {
      ...
      let componentForCreate = _.cloneDeep(props.canvasStore.componentsData.find(i => i.componentId == componentId));
      ...
      props.canvasStore.addCanvasData(
        componentForCreate
      );
    } else {
      let uuid = e.dataTransfer.getData("uuid")
      let componentForDrag = props.canvasStore.canvasData.find(i => i.uuid == uuid);
        ...
        <div className='sider'>
          {/* 渲染组件 */}
          {
            selectedKey == 1 && <div>
              ...
              const {data: {type, props, children}, componentId, desc } = i
          ...
          >
            {/* 渲染画布 */}
            {
              props?.canvasStore?.canvasData.map((i: ComponentItem) => {
                const { data: { type, props, children }, style, uuid } = i;
                ...
                >
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      const comp = _.cloneDeep(props.canvasStore.canvasData.find(i => i.uuid === selectedComponent.uuid));
                      if (i.key === 'children') {
                    ...
                    defaultValue={(() => {
                      ...
                      if (i.key === 'children') {
                        ...
                        if (i.suffix) {
                          return (comp.data.props[i?.key] + '').split(i.suffix)[0]
                        } else {
